...
class SpdlogConan(ConanFile):
    ...
    def generate(self):
        if not self.options.header_only:
            ...
            tc.variables["SPDLOG_CLOCK_COARSE"] = self.options.clock_coarse
            tc.variables["SPDLOG_PREVENT_CHILD_FD"] = self.options.prevent_child_fd
            tc.variables["SPDLOG_NO_THREAD_ID"] = self.options.no_thread_id
            tc.variables["SPDLOG_NO_ATOMIC_LEVELS"] = self.options.no_atomic_levels
            tc.variables["SPDLOG_DISABLE_DEFAULT_LOGGER"] = self.options.disable_default_logger
            tc.variables["SPDLOG_USE_STD_FORMAT"] = self.options.use_std_format
